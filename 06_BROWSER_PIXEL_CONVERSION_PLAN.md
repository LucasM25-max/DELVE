...
| Version | 2.0 (2026-09-23) — HTML + ES modules + JSON + PNG/OGG |
...
engine input map + autoloads        →      core/input.js + core/state.js
...
| Gates | 5 commands (2 Python, 3 Node) | `tools/`, `web/tests/` |
...
| Page images | `node web/tests/shoot.mjs` | rasterises the same pages through a pixel-true canvas (real `drawImage`, alpha and composite modes) and writes `web/preview/shell-*.png` — the pictures CI attaches to the run |
...
stills used for review; the CI log holds the gate output; the PR description links the Vercel
preview.
...
└ web/                      the game (see web/README.md)
   ...
   ├ tests/                    run.mjs · smoke.mjs · shoot.mjs
