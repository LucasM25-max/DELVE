...
Both are emitted as AngelCode BMFont (`.fnt` + a 1-bpp-ish white-on-transparent
PNG atlas), which Godot 4 imports as a bitmap `FontFile`. No TTF is rasterised
at runtime anywhere in the game build.
...
SOURCES = os.path.join(ROOT, "tools", "sources")
