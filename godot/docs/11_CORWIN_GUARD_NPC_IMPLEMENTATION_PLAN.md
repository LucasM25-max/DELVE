...
| Step 4 integration | generator adds ext `43_corwin` (renumbered past the ground-GLB ids in the main merge) and `NPCs/Corwin` at ROT180 (2.5, 0.16, 1.5); `load_steps` 68 (43 ext + 24 sub + 1), now computed and self-checked at generation — both branches had independently fixed the base header's off-by-one (70 vs 66). |
...
| Step 6 QA | `tools/qa_yard.gd` grows 16 → 41 checks on this branch (51 after the main merge folded in the ground checks): the planned static checks plus runtime behaviour probes (lean/scan pose diffs on real bones, scan-machine sweep, posed-hand fit inside the box, trigger one-shot + overlap refusal + VO playback, bed duck). |
...
0 failures) + `SMOKE_ALL_GREEN` (80 checks, 0 failures). After the Round 15
merge with main (PR #6 displaced ground): `QA_YARD_PASS` (51 checks,
