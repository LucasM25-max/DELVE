...
## Round 15 — main merge: displaced ground + Corwin milestone (2026-09-23)

Scope: merge `origin/main` (1aa9819 — PR #6 "Training yard: 3D heightfield
floor + polished tileable ground textures") into the Corwin/TR_A_LANE branch
for PR #7. Twelve conflicts resolved with no scope dropped on either side:
the six `T_YRD_*.png.import` sidecars take main's versions (their checksums
track the polished PNGs); `tools/scene/build_test_yard.py` unifies both
builders (Corwin ext renumbered `43_corwin` past the ground-GLB ids
`41_ground_yard`/`42_ground_forecourt`; `load_steps` is now computed and
self-checked against the emitted ext+sub lines = 68: 43 ext + 24 sub + 1 —
both branches had independently fixed the base header's off-by-one, 70 vs
66) and regenerates `scenes/world/test_yard.tscn` (216 node blocks; 64
CollisionShape3D nodes: 61 Level + lane sphere + 2 ground trimesh slots);
`tools/qa_yard.gd` unions main's ground checks with the Corwin/TR_A_LANE +
runtime behaviour probes; `tests/smoke_test.gd` takes the displaced-GLB
ground probe (`Terrain3D/DirtGround/Ground/M_YRD_GROUND_YARD` material,
vertex tints) with the merged node budget (<= 8 = 7 scene roots + runtime
HUD); `tests/yard_shot.gd` keeps both capture sets (r2 ground walk + r3
Corwin, shared null-safe `shot()`); docs/10 status/scope merge both
extensions.

Result: **QA_YARD_PASS — checks run: 51, failures: 0** on the merged tree
(main's 31-check suite unioned with this branch's 41-check suite over the
shared core). **DELVE SMOKE TEST: 80 checks, 0 failures / SMOKE_ALL_GREEN**
(exit 0). `tests/yard_shot.tscn` → **YARD_SHOTS_DONE**: r2-01..09 and
r3-01/02/03 all report `SHOT_SKIP (no renderer)` under the headless dummy
renderer — windowed capture still needs a display machine.

Tooling: editor binary rebuilt via `tools/rebuild_godot_headless.sh` after
the sandbox cache reset (Godot 4.7.2-stable source, x11=no, gold link);
`--import` regenerated the merged asset set cleanly. Corwin placement and
the TR_A_LANE probe set are unchanged from Round 14; the heightfield relief
(+/-0.2 m envelope, QA-asserted) sits under the fixed props exactly as on
main.
