...
def build() -> str:
    ...
    d = Doc()
    # 34 ext resources + 24 sub resources + 1
    d.w('[gd_scene load_steps="59" format=3 uid="uid://delve-testyard-02"]')
    d.w()
    ...
    d.w('[sub_resource type="WorldBoundaryShape3D" id="GroundShape"]')
    d.w()
    d.w('[sub_resource type="StandardMaterial3D" id="GroundMat"]')
    d.w("albedo_color = Color(0.96, 0.96, 0.96, 1)")
    d.w("roughness = 0.9")
    d.w()
    d.w('[sub_resource type="PlaneMesh" id="GroundMesh"]')
    d.w('material = SubResource("GroundMat")')
    d.w("size = Vector2(400, 400)")
    d.w()
    ...
    d.w("albedo_color = Color(1, 0.72, 0.42, 1)")
    d.w("roughness = 0.4")
    ...
    d.w("albedo_color = Color(0.09, 0.07, 0.05, 1)")
    d.w("roughness = 0.95")
    d.w()
    ...
    d.w()
    d.w('[node name="Ground" type="StaticBody3D" parent="."]')
    d.w()
    d.w('[node name="Mesh" type="MeshInstance3D" parent="Ground"]')
    d.w('mesh = SubResource("GroundMesh")')
    d.w()
    d.w('[node name="Collision" type="CollisionShape3D" parent="Ground"]')
    d.w('shape = SubResource("GroundShape")')
    d.w()
