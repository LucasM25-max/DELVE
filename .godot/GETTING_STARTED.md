...
Two plugin families and two content kits are now part of the production plan.

| Item | Type | License | Where it lives | Status |
| --- | --- | --- | --- | --- |
| Terrain3D (Tokisan Games) | GDExtension plugin | MIT | `addons/terrain_3d` (you install) | adopted, not yet in scenes |
| Sky3D (Tokisan Games) | GDScript plugin | MIT | `addons/sky_3d` (you install) | adopted, not yet in scenes |
| Universal Animation Library (Quaternius) | animation kit, CC0 | CC0 | `assets/characters/universal/` (committed) | **live** — drives the player |
| Universal Base Characters (Quaternius) | humanoid models, CC0 | CC0 | `assets/characters/hero/` (committed) | **live** — the player body |

Install/enable the plugins:

1. Godot → **AssetLib** tab → search *Terrain3D* / *Sky3D* → Download → Install.
   (Or copy `addons/…` from their GitHub releases — same builds.)
2. **Project → Project Settings → Plugins** → enable both.
3. Restart the editor once. Terrain3D adds a terrain editor dock; Sky3D adds a Sky3D node
   type that replaces a scene's `WorldEnvironment`.

Notes that matter later:

- The project commits its **own CC0 copy** of the animation kit and base characters so any
  clone builds identically; an Asset-Store copy of the animation kit on your disk is
  equivalent but unused by the scenes.
- Terrain3D 1.0.2 officially targets Godot 4.4–4.6; it was load- and render-verified on
  4.7.2 with our Compatibility renderer (see `tests/TEST_REPORT.md`). **Web export of
  Terrain3D is experimental upstream** — plan terrain for desktop builds first.
- glTF import trims `_Loop` suffixes from animation names (`Walk_Loop` → `Walk`); the
  player script maps states through the `ANIM_*` constants for that reason.
- Clothing/hair for the base bodies is the next art pass (Quaternius Modular Outfits CC0
  or Blender cloth). The bodies ship as base meshes by design.

## 9. Keeping GitHub and Godot in sync

Follow **[GITHUB_SYNC.md](GITHUB_SYNC.md)**: close Godot → pull (or merge a delivery zip) →
reopen → let it import → F5 → commit → push. The guide covers GitHub Desktop and terminal
routes, plugin survival, and the troubleshooting table.
