...
Outputs, under `web/assets/pixel/ui/`, plus the machine-readable
`ui_kit.json` that the UI kit reads at boot (`web/js/ui/kit.js`):
...
Every panel source is a 48x48 nine-patch. The kit blits the four corners 1:1
and tiles the middle, so the corner art stays crisp at any panel size
(GDD-07 §7.4) — the same contract, done with canvas draws instead of a theme.
